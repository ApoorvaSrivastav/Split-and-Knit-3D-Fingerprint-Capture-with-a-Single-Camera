function Data_points = Metrics_single(Score,n_fin,num_threshold)
GT =eye(n_fin);
threshold = linspace(0,1,num_threshold);
Data_points = zeros(num_threshold,4);
%color = ['r','g','b'];
%color = ['r','g'];
% EER = zeros(4,1);
...
%for c=1:size(Score,3)
       for i =1:length(threshold)
        ...
        Count_FN =0;
        Count_TN =0;
        Prediction = imbinarize(Score,threshold(i));
        for j= 1:size(GT,1)
            for k= 1:size(GT,2)
        ...
        TRR = 1-FAR;%correct
        TAR = 1-FRR;%correct
        Data_points(i,:) = [FAR,TAR,FRR,TRR];

       end
    semilogx(Data_points(:,1),Data_points(:,2),'Color','r');
    hold all
